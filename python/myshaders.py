# myshaders: modification of pyshaders to not depend on pyglet
import trace
...
from OpenGL.GL import (glCreateShader, glShaderSource, glCompileShader,
  ...
  glDisableVertexAttribArray, glGetVertexAttribiv)

#from OpenGL.raw.GL.VERSION.GL_2_0 import glVertexAttribPointer
from pyglet.gl import glVertexAttribPointer

from OpenGL.GL import (GL_VERTEX_SHADER, GL_FRAGMENT_SHADER, GL_COMPILE_STATUS,
...
class ShaderAttribute(object):
    ...
    def point_to(self, offset, type, size, normalized=False, stride=0):
        ...
        """
        #print("loc",self.loc, "offset",offset)
        #glVertexAttribPointer(self.loc, size, type, (normalized), c_int(stride), c_int(offset))
...
class ShaderAttributeAccessor(ShaderAccessor):
    ...
    def reload(self):
        ...
        for i in range(getattr(prog, 'attributes_count')):
            glGetActiveAttrib(prog.pid, i, maxlength, byref(name_buf_length), byref(buf_size), byref(type_buf), name_buf_ptr)
            type = type_buf.value
            name_len = name_buf_length.value
            name = bytes(name_buf)[0:name_len].decode('UTF-8')
            size = buf_size.value 
            
            loc = GLint(glGetAttribLocation(prog.pid, name))   #Kept in a c_int to quickly send the value when setting  
...
class ShaderUniformAccessor(ShaderAccessor):
    ...
    def cache_item_build(self, loc, size, name, type):
        ...
        if size == 1 and not '[0]' in name:                         #Arrays name ends with [0]. Ex: 'int[5] foo' name is 'foo[0]'
            ...
            is_array = True
        is_array = False
        print("c_i_b uniform: name",name,"loc",loc)
